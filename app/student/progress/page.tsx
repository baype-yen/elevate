...
export default function ProgressPage() {
  ...
  if (loading || !data) {
    return <div className="font-sans text-sm text-text-mid">Loading progress report...</div>
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="bg-navy rounded-[20px] px-7 py-6 flex justify-between items-center">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="font-sans text-xs text-gray-mid">Updated from your latest Supabase records</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-6">
        ...
        <div className="bg-off-white rounded-2xl border border-gray-light p-5">
          ...
          <div className="flex items-end gap-1.5 h-[120px] pt-2.5">
            {(data.scoreEvolution.length ? data.scoreEvolution : [{ month: "Now", score: 0 }]).map((m: any, i: number) => (
      ...
      <div>
        ...
        <div className="bg-off-white rounded-[14px] border border-gray-light overflow-hidden">
          ...
          {!data.recentGrades.length && <div className="px-4 py-3 font-sans text-sm text-text-mid">No graded work yet.</div>}
        </div>
      </div>
      ...
      <div className="p-5 rounded-[14px] bg-violet/5 border border-violet-pale">
        <div className="flex items-center gap-2.5 mb-2.5">
          ...
          <div>
            <div className="font-sans text-[13px] font-semibold text-navy">Teacher Feedback — {data.feedback?.teacher || "No feedback yet"}</div>
        ...
        <p className="font-sans text-sm text-text-dark leading-relaxed pl-[46px]">
          {data.feedback?.text || "Your teacher feedback will appear here once published."}
        </p>
      </div>
    </div>
  )
